...
exports.sendLoyaltyEmailController = async (req, res) => {
  ...
  try {
    await emailService.sendLoyaltyEmail(
      recipient,
      customerName || "Valued Customer"
    );
    res
      .status(200)
      .json({
        message: `Order confirmation (offer used) email sent to ${recipient}`,
      });
  } catch (error) {
...
exports.sendAbandonedOfferEmailController = async (req, res) => {
  ...
  try {
    await emailService.sendAbandonedOfferEmail(
      recipient,
      customerName || "Valued Customer",
      orderNumber || "BD-XXYYZZ"
    );
    res
      .status(200)
      .json({
        message: `Order confirmation (offer not used) email sent to ${recipient}`,
      });
  } catch (error) {
...
exports.sendOrderProcessingEmailController = async (req, res) => {
  ...
  if (!recipient || !orderNumber) {
    return res
      .status(400)
      .json({ message: "Recipient email and order number are required." });
  }
  try {
    ...
    res
      .status(200)
      .json({ message: `Order processing email sent to ${recipient}` });
  } catch (error) {
...
exports.sendOrderHandlingEmailController = async (req, res) => {
  ...
  if (!recipient || !orderNumber || !productName) {
    return res
      ...
      .json({
        message:
          "Recipient email, order number, and product name are required.",
      });
  }
  try {
    await emailService.sendOrderHandlingEmail(
      recipient,
      customerName || "Valued Customer",
      orderNumber,
      productName
    ...
    res
      .status(200)
      .json({ message: `Order handling email sent to ${recipient}` });
  } catch (error) {
...
exports.sendOrderShippedEmailController = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Order shipped email sent to ${recipient}` });
  } catch (error) {
...
exports.sendOrderDeliveredEmailController = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Order delivered email sent to ${recipient}` });
  } catch (error) {
...
exports.sendOrderCancelledEmailController = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Order cancelled email sent to ${recipient}` });
  } catch (error) {
...
exports.sendForgotPasswordEmailController = async (req, res) => {
  ...
  if (!recipient) {
    return res
      .status(400)
      .json({ message: "Email address is required for password reset." });
  }
  try {
    ...
    res
      .status(200)
      .json({ message: `Password reset email sent to ${recipient}` });
  } catch (error) {
...
exports.sendPasswordChangeConfirmationEmailController = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: `Password change confirmation email sent to ${recipient}`,
      });
  } catch (error) {
    handleError(
      res,
...
exports.sendCustomerInquiryAutoReplyEmailController = async (req, res) => {
  ...
  if (!recipient || !originalCustomerMessage) {
    return res
      .status(400)
      .json({ message: "Recipient email and original message are required." });
  }
  try {
    await emailService.sendCustomerInquiryAutoReplyEmail(
      recipient,
      customerName || "Valued Customer",
    ...
    res
      .status(200)
      .json({
        message: `Customer inquiry auto-reply email sent to ${recipient}`,
      });
  } catch (error) {
...
// 5.2. Contact Form Submission (to Admin)
exports.sendContactFormEmailController = async (req, res) => {
  const { name, email, message } = req.body;
  const adminRecipient = "brandsoverstockinfo@gmail.com";
  ...
  try {
    ...
    await emailService.sendCustomerInquiryAutoReplyEmail(email, name, message);
    ...
    handleError(res, error, "Failed to send contact form email");
  }
};
...
exports.sendAdminPlatformSaleAlertController = async (req, res) => {
  ...
  if (!recipient || !orderNumber || !sku) {
    return res
      .status(400)
      .json({ message: "Admin email, order number, and SKU are required." });
  }
  const data = {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: `Admin platform sale alert email sent to ${recipient}`,
      });
  } catch (error) {
...
exports.sendAdminPhysicalStoreSaleConfirmationController = async (req, res) => {
  const { adminEmail, sku, title, quantitySold, remainingStock, itemLocation } =
    req.body;
  const recipient = adminEmail || process.env.TEST_ADMIN_EMAIL;
  if (!recipient || !sku) {
    return res
      .status(400)
      .json({ message: "Admin email and SKU are required." });
  }
  const data = {
    sku: sku,
    title: title || "Product Title",
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: `Admin physical store sale confirmation email sent to ${recipient}`,
      });
  } catch (error) {
    handleError(
      res,
      error,
      "Failed to send admin physical store sale confirmation email"
    );
  }
};
...
exports.sendAdminInventorySyncFailureAlertController = async (req, res) => {
  ...
  if (!recipient || !sku || !platformName || !errorMessage) {
    return res
      ...
      .json({
        message:
          "Admin email, SKU, platform name, and error message are required.",
      });
  }
  const data = {
    sku: sku,
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: `Admin inventory sync failure alert email sent to ${recipient}`,
      });
  } catch (error) {
    handleError(
      res,
      error,
      "Failed to send admin inventory sync failure alert email"
    );
  }
};
...
exports.sendOrderUpdateEmailController = async (req, res) => {
  ...
  if (!recipient || !orderNumber || !message) {
    return res
      .status(400)
      .json({
        message: "Recipient email, order number, and message are required.",
      });
  }
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Order update email sent to ${recipient}` });
  } catch (error) {
