...
const createProduct = async (req, res) => {
  try {
        ...
        ebayPromises.push(
          createEbayProduct(eBayProductForService, globalRequiredFields)
            .then((res) => {
              if (res.success) {
                console.log(
                  "Successfully created product on eBay1 platform:",
                  res.data
                );
        ...
        ebayPromises.push(
          createEbayProduct2(eBayProductForService, globalRequiredFields)
            .then((res) => {
              if (res.success) {
                console.log(
                  "Successfully created product on eBay2 platform:",
                  res.data
                );
        ...
        ebayPromises.push(
          createEbayProduct3(eBayProductForService, globalRequiredFields)
            .then((res) => {
              if (res.success) {
                console.log(
                  "Successfully created product on eBay3 platform:",
                  res.data
                );
    ...
    const ebayResults = Object.entries(eBayResponses).filter(
      ([key]) => key !== "generalError"
    );
    const ebaySuccessCount = ebayResults.filter(
      ([, result]) => result.success
    ).length;
    const ebayErrorCount = ebayResults.filter(
      ([, result]) => !result.success
    ).length;
    ...
    if (totalEbayAttempts > 0) {
      if (ebaySuccessCount === totalEbayAttempts && !hasGeneralError) {
        ...
        const successfulPlatforms = ebayResults
          ...
          .map(
            ([platform, result]) =>
              `${platform} (Offer ID: ${result.data?.offerId || "N/A"})`
          )
          .join(", ");
    ...
    if (hasGeneralError && ebaySuccessCount > 0) {
      warnings.push(
        `eBay integration warning: ${eBayResponses.generalError.error}`
      );
    }
...
const updateProductQuantities = async (req, res) => {
    ...
    try {
      ...
      if (updatedRecord) {
        ...
        if (currentProductDetails.ebayOne) {
          syncPromises.push(
            ebayUpdateStock(currentSku, stock_quantity_for_external, quantity)
          );
        }
        if (currentProductDetails.ebayTwo) {
          syncPromises.push(
            ebayUpdateStock2(currentSku, stock_quantity_for_external, quantity)
          );
        }
        if (currentProductDetails.ebayThree) {
          syncPromises.push(
            ebayUpdateStock3(currentSku, stock_quantity_for_external, quantity)
          );
        }
        ...
        if (failedSyncs.length > 0) {
          console.error(
            `Some external syncs failed for SKU ${currentSku}:`,
            failedSyncs
          );
  ...
  if (allSuccessful && results.length > 0) {
    ...
    return res.status(207).json({
      success: false,
      message:
        "Some products failed stock deduction. Check results for details.",
      results,
    });
  }
};
