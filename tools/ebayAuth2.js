...
const EBAY2_CLIENT_ID = process.env.EBAY2_CLIENT_ID;
const EBAY2_CLIENT_SECRET = process.env.EBAY2_CLIENT_SECRET;
const EBAY2_REDIRECT_URI = process.env.EBAY2_REDIRECT_URI;

// 🔑 Get Authorization URL
function getEbayAuthUrl2() {
  const scope = [
  ...
  const params = {
    client_id: EBAY2_CLIENT_ID,
    response_type: "code",
    redirect_uri: EBAY2_REDIRECT_URI,
    scope,
  };
...
async function getAccessToken2(authCode) {
  const authHeader = `Basic ${Buffer.from(
    `${EBAY2_CLIENT_ID}:${EBAY2_CLIENT_SECRET}`
  ).toString("base64")}`;
...
async function refreshAccessToken2() {
  ...
  try {
    ...
    if (error.response?.data?.error === "invalid_grant") {
      ...
      //   .delete({ where: { platform: Platform.EBAY2 } })
      //   .catch(() => {});
      throw new Error(
...
async function getValidAccessToken2() {
  ...
  return await refreshAccessToken2();
}
