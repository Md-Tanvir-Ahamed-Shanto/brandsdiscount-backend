...
try {
  ...
  if (apiKey.startsWith("sk_live_") && apiKey.length > 100) {
    console.warn(
      "Warning: Stripe live key appears to be malformed or contains extra characters"
    );
  }
  ...
  console.log(
    "Stripe initialized successfully with key type:",
    cleanKey.startsWith("sk_live") ? "live" : "test"
  );
} catch (error) {
...
const { sendOrderRefundedEmail } = require("../tools/email");
...
router.post("/create-checkout-session", async (req, res) => {
  try {
      ...
      return {
        price_data: {
          ...
          product_data: {
            ...
            description: `${item.brandName || ""} - ${item.color || ""} - ${
              item.sizeType || item.size || ""
            }`
              .trim()
              .substring(0, 300), // Stripe limit
    ...
    if (Math.abs(calculatedTotal - finalAmount) > 0.01) {
      ...
      console.warn(
        `Amount mismatch: calculated ${calculatedTotal}, provided ${finalAmount}`
      );
    }
    ...
    if (error.type === "StripeCardError") {
      ...
      statusCode = 400;
      errorMessage =
        "Invalid request parameters. Please check your cart and try again.";
      ...
      statusCode = 400;
      errorMessage =
        "Invalid product pricing. Please refresh your cart and try again.";
  ...
  try {
    ...
    if (!validReasons.includes(reason)) {
      return res.status(400).json({
        success: false,
        error:
          "Invalid refund reason. Must be one of: duplicate, fraudulent, requested_by_customer",
      });
    }
    ...
    if (orderId && refund.status === "succeeded") {
      try {
        ...
        await sendOrderRefundedEmail(
          ...
          orderId,
          refund?.orderDetails?.map((detail) => ({
            sku: detail?.sku || "N/A",
          ...
          refund?.amount ? refund?.amount / 100 : 0
        );
